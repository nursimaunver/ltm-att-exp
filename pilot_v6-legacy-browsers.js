...
let expName = 'pilot_v6';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  accuracy_2Clock = new util.Clock();
...
function trials_practiceLoopBegin(trials_practiceLoopScheduler) {
  ...
  trials_practice.forEach(function() {
    ...
    trials_practiceLoopScheduler.add(accuracy_2RoutineBegin(snapshot));
    trials_practiceLoopScheduler.add(accuracy_2RoutineEachFrame(snapshot));
    trials_practiceLoopScheduler.add(accuracy_2RoutineEnd(snapshot));
...
var acc;
var accuracy_2Components;
function accuracy_2RoutineBegin(snapshot) {
  return function () {
    //------Prepare to start Routine 'accuracy_2'-------
    t = 0;
    accuracy_2Clock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    acc = 0;
    ...
    accuracy_2Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function accuracy_2RoutineEachFrame(snapshot) {
  return function () {
    //------Loop for each frame of Routine 'accuracy_2'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
...
function accuracy_2RoutineEnd(snapshot) {
  return function () {
