...
expName = 'pilot_v7.1'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/nursimaunver/Memory ryhthms/ltm-att-exp/pilot_v7.1_lastrun.py',
...
polygon_out2 = visual.Rect(
    win=win, name='polygon_out2',
    width=(0.82, 0.82)[0], height=(0.82, 0.82)[1],
    ori=0, pos=(0, 0),
    lineWidth=1, lineColor=[-1,-1,-1], lineColorSpace='rgb',
    fillColor=[-1,-1,-1], fillColorSpace='rgb',
    opacity=0.5, depth=0.0, interpolate=True)
...
for thisTrials_out in trials_out:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_in = data.TrialHandler(nReps=99, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('cond.xlsx', selection=wrong_list),
        seed=None, name='trials_in')
    thisExp.addLoop(trials_in)  # add the loop to the experiment
    thisTrials_in = trials_in.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_in.rgb)
    if thisTrials_in != None:
        for paramName in thisTrials_in:
            exec('{} = thisTrials_in[paramName]'.format(paramName))
    
    for thisTrials_in in trials_in:
        currentLoop = trials_in
        # abbreviate parameter names if possible (e.g. rgb = thisTrials_in.rgb)
        if thisTrials_in != None:
            for paramName in thisTrials_in:
                exec('{} = thisTrials_in[paramName]'.format(paramName))
        ...
        #if trials_out.thisN == 0 and trials_in.thisN == 0:
            #wrong_list = []
        if trials_out.thisN  >0 :
            if row_id not in wrong_list:
                continueRoutine = False
                trials_in.finished=True
        ...
        for thisComponent in practice_blockComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        trials_in.addData('polygon_outside2.started', polygon_outside2.tStartRefresh)
        trials_in.addData('polygon_outside2.stopped', polygon_outside2.tStopRefresh)
        trials_in.addData('polygon_inside2.started', polygon_inside2.tStartRefresh)
        trials_in.addData('polygon_inside2.stopped', polygon_inside2.tStopRefresh)
        trials_in.addData('text_3.started', text_3.tStartRefresh)
        trials_in.addData('text_3.stopped', text_3.tStopRefresh)
        trials_in.addData('image_2.started', image_2.tStartRefresh)
        trials_in.addData('image_2.stopped', image_2.tStopRefresh)
        # store data for trials_in (TrialHandler)
        x, y = mouse_4.getPos()
        buttons = mouse_4.getPressed()
        if sum(buttons):
            # check if the mouse was inside our 'clickable' objects
            ...
            for obj in [polygon_outside2]:
                if obj.contains(mouse_4):
                    ...
                    mouse_4.clicked_name.append(obj.name)
                    mouse_4.clicked_pos.append(obj.pos)
        trials_in.addData('mouse_4.x', x)
        trials_in.addData('mouse_4.y', y)
        trials_in.addData('mouse_4.leftButton', buttons[0])
        trials_in.addData('mouse_4.midButton', buttons[1])
        trials_in.addData('mouse_4.rightButton', buttons[2])
        if len(mouse_4.clicked_name):
            trials_in.addData('mouse_4.clicked_name', mouse_4.clicked_name[0])
        if len(mouse_4.clicked_pos):
            trials_in.addData('mouse_4.clicked_pos', mouse_4.clicked_pos[0])
        trials_in.addData('mouse_4.started', mouse_4.tStart)
        trials_in.addData('mouse_4.stopped', mouse_4.tStop)
        ...
        trials_in.addData('polygon_outside3.started', polygon_outside3.tStartRefresh)
        trials_in.addData('polygon_outside3.stopped', polygon_outside3.tStopRefresh)
        trials_in.addData('polygon_inside3.started', polygon_inside3.tStartRefresh)
        trials_in.addData('polygon_inside3.stopped', polygon_inside3.tStopRefresh)
        trials_in.addData('polygon.started', polygon.tStartRefresh)
        trials_in.addData('polygon.stopped', polygon.tStopRefresh)
        trials_in.addData('feedback_Txt.started', feedback_Txt.tStartRefresh)
        trials_in.addData('feedback_Txt.stopped', feedback_Txt.tStopRefresh)
        trials_in.addData('image_3.started', image_3.tStartRefresh)
        trials_in.addData('image_3.stopped', image_3.tStopRefresh)
        trials_in.addData('polygon_4.started', polygon_4.tStartRefresh)
        trials_in.addData('polygon_4.stopped', polygon_4.tStopRefresh)
        trials_in.addData('text_10.started', text_10.tStartRefresh)
        trials_in.addData('text_10.stopped', text_10.tStopRefresh)
        thisExp.nextEntry()
        
    # completed 99 repeats of 'trials_in'
    
    
    # ------Prepare to start Routine "checker"-------
    continueRoutine = True
    # update component parameters for each repeat
    if trials_out.thisN != 0 and len(wrong_list) ==0:
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    checkerClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "checker"-------
    while continueRoutine:
        # get current time
        t = checkerClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=checkerClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    for thisComponent in checkerComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "checker" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisSound_loop in sound_loop:
    ...
    sound_3.setSound(sounds1, secs=3, hamming=True)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if key_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_2.getKeys(keyList=['a', 's'], waitRelease=False)
...
for thisDist_loop in dist_loop:
    ...
    # keep track of which components have finished
    dist_imgComponents = [polygon_out2, polygon_in2, ret, text_5]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if polygon_out2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > polygon_out2.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if polygon_in2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > polygon_in2.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in dist_imgComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    dist_loop.addData('polygon_out2.started', polygon_out2.tStartRefresh)
    dist_loop.addData('polygon_out2.stopped', polygon_out2.tStopRefresh)
    dist_loop.addData('polygon_in2.started', polygon_in2.tStartRefresh)
    dist_loop.addData('polygon_in2.stopped', polygon_in2.tStopRefresh)
