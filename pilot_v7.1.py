...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/nursimaunver/Memory ryhthms/ltm-att-exp/pilot_v7.1.py',
...
for thisTrials_out in trials_out:
    ...
    for thisTrials_in in trials_in:
        ...
        if trials_prac2.thisN ==0 and degree_difference>20: 
...
for thisSound_loop in sound_loop:
    ...
    _dual_resp2_allKeys = []
    # keep track of which components have finished
    sound2Components = [sound_2, dual_resp2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if dual_resp2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > dual_resp2.tStartRefresh + 1.5-frameTolerance:
                # keep track of stop time/frame for later
                dual_resp2.tStop = t  # not accounting for scr refresh
                dual_resp2.frameNStop = frameN  # exact frame index
                win.timeOnFlip(dual_resp2, 'tStopRefresh')  # time at next scr refresh
                dual_resp2.status = FINISHED
