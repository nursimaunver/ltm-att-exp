...
expName = 'pilot_v4'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/nursimaunver/Memory ryhthms/ltm-att-exp/pilot_v4_lastrun.py',
...
for thisTrials_loop in trials_loop:
    ...
    # keep track of which components have finished
    show_imagesComponents = [image_10, Stim_image]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if image_10.status == STARTED:
            ...
            if tThisFlipGlobal > image_10.tStartRefresh + 0.1-frameTolerance:
...
for thisTrials_practice in trials_practice:
    ...
    degree_difference = round((resp_degree - actual_degree),2)
    
    polygon_4.setPos((study_location_x,study_location_y))
    feedback_Txt.setPos((study_location_x,study_location_y))
    feedback_Txt.setText(degree_difference

)
    image_13.setImage('circle.png')
    # keep track of which components have finished
    feedbackComponents = [polygon_4, feedback_Txt, image_13]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if feedback_Txt.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > feedback_Txt.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
    ...
    trials_practice.addData('image_13.started', image_13.tStartRefresh)
    trials_practice.addData('image_13.stopped', image_13.tStopRefresh)
    thisExp.nextEntry()
